export default function StatCards({ profile, submissionCount, problemCount }) {
  const stats = [
    { label: '当前干员', value: profile?.username || 'GUEST', tone: 'cyan', code: 'USR', sub: profile?.role || 'visitor' },
    { label: '已解决', value: profile?.solved_count ?? 0, tone: 'red', code: 'AC', sub: `Rating ${profile?.rating || 1200}` },
    { label: '通过提交', value: profile?.accepted_count ?? 0, tone: 'amber', code: 'SUB', sub: `${profile?.submit_count ?? submissionCount} total` },
