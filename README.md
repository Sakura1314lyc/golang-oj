...
一个基于 Go + React 的在线判题系统（Online Judge）。前端采用战术终端风格界面，后端提供完整的题库、提交、判题、排行榜、比赛和公告等 REST API。

## 技术栈

| 层级 | 技术 |
| --- | --- |
| 后端 | Go 1.26, GORM, MySQL |
| 前端 | React 19, Vite, Tailwind CSS v4 |
| 编辑器 | CodeMirror 6 |
| 判题 | 本地沙箱内编译/运行 C++ / Go / Python / Java / JavaScript |
| 认证 | JWT（HMAC-SHA256, 72h 过期） |

## 功能特性

- **题目管理** — 28 道原创题 + 120 道 Codeforces 导入题，支持搜索 / 标签过滤 / 分页
- **多语言支持** — C++17、Go、Python、Java、JavaScript，各自带默认代码模板
- **真实判题** — 编译执行 + 逐测试点比对 + 超时控制 + 内存限制
- **用户系统** — 注册 / 登录 / JWT 鉴权 / 资料修改 / 密码修改 / 角色权限
- **个人面板** — 档案下拉菜单，展示解题数、Rating、提交统计
- **排行榜** — 按评分 / 解题数排序，分页展示
...
- **API 安全** — 速率限制（内存中按 IP）、请求日志、CORS 配置
- **优雅关停** — 信号监听（SIGINT/SIGTERM），安全退出
- **暗色 / 亮色模式** — 一键切换，沉浸式战术终端主题
...
- Node.js 18+
- MySQL 8.0+
- g++（C++17）、Python 3、Java JDK（可选）、Node.js（可选，用于 JavaScript 判题）
...
# 自动建表 + 种子数据（28 道原创题、测试用例、默认用户、比赛）
```
...
项目自带的 28 道原创题之外，额外提供从 Codeforces 导入题目的工具：
...
默认导入 rating 800–2000 范围的 120 道题目。可在 `main.go` 中调整导入数量和范围。
...
| 路由 | 方法 | 说明 |
| --- | --- | --- |
| `/api/admin/problems/create` | POST | 创建题目 |
| `/api/admin/problems/:id` | PUT | 更新题目 |
| `/api/admin/problems/:id` | DELETE | 删除题目 |
| `/api/admin/users` | GET | 用户列表（分页） |
| `/api/admin/rejudge/:id` | POST | 重新判题 |
| `/api/admin/stats/problems` | GET | 全题目统计 |
| `/api/admin/stats/problems/:id` | GET | 单题统计 |
| `/api/admin/announcements` | GET | 公告管理列表 |
| `/api/admin/announcements/create` | POST | 发布公告 |
...
| 语言 | 编译/运行方式 | 默认模板 |
| --- | --- | --- |
| C++ | `g++ -std=c++17 -O2` → 运行二进制 | 标准 I/O |
| Go | `go run`（自动生成临时 `go.mod`） | 标准 I/O |
| Python | `python` 解释执行 | 标准 I/O |
| Java | `javac` → `java` | Scanner |
| JavaScript | `node` 解释执行 | readline |

### 判题流程

1. 编译（C++/Java 需编译，Go/Python/JS 直接运行）
2. 逐测试点执行，输入重定向
3. 捕获输出，与预期结果逐字符比对（忽略末尾空白）
4. 超时检测：每个测试点独立超时（题目时限 + 2s 缓冲）
5. 返回判题结果
...
│   └── ratelimit.go        # 速率限制
├── judge/
│   └── judge.go            # 判题引擎（编译+运行+比对）
├── routes/
│   └── routes.go           # 路由注册
├── seed/
│   └── seed.go             # 种子数据
├── tools/
│   └── fetch_problems/     # Codeforces 题目导入工具
└── gooj-frontend/          # 前端项目
    ├── public/
    └── src/
        ├── App.jsx         # 主入口（路由、状态）
        ...
        ├── components/
        │   ├── layout/     # 布局组件（Header）
        │   ├── dashboard/  # 仪表盘（题目/排行榜/提交/比赛/统计）
        │   ├── problem/    # 题目详情 + CodeMirror 编辑器
        │   ├── submissions/# 个人提交记录页
        │   ├── auth/       # 登录/注册模态框
        │   └── common/     # 通用组件（ErrorBoundary、LoadingSkeleton）
        └── index.css       # Tailwind + 自定义主题 + 战术终端风格样式
```
...
# 前端构建
cd gooj-frontend && npm run build

# 前端 lint
cd gooj-frontend && npm run lint
```
